...
import { updateStart, updateSuccess
  ,updateFailure
...
const DashProfile = () => {
  ...
  function handleImageChange(e) {
    ...
    if (file) {
      setImageFile(file);
      setimageFileUrl(URL.createObjectURL(file)); //create url for us
    }
  }
  ...
  const uploadImage = async () => {
    ...
    uploadTask.on(
      ...
      (error) => {
        setImageFileUploadError("Could not upload image must less than 2MB");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if(!res.ok){
        ...
        setUpdateUserSuccess("User Updated Succesfully");
      }
